...
import { Pause, Play, Radio, RotateCcw, RotateCw, Gauge, Maximize, Minimize, ChevronDown, X } from "lucide-react"
...
export function EpisodePlayer({
  ...
  onMinimize?: () => void
  onRestore?: () => void
  /** Closes/exits the video entirely (top-right ✕). */
  onClose?: () => void
}) {
  ...
  const renderVideoOverlay = () => (
    ...
    >
      ...
      {/* Minimize (top-left) — collapses into a YouTube-style floating mini-player.
          Icon only (no circle/pill); a drop-shadow keeps it legible on video. */}
      {onMinimize && (
        <button
          type="button"
          onClick={(e) => {
          ...
          className="absolute left-3 top-3 z-20 flex items-center justify-center text-white drop-shadow-[0_1px_3px_rgba(0,0,0,0.6)] transition-transform hover:text-white active:scale-90"
        >
          <ChevronDown className="size-7" />
        </button>
      )}

      {/* Close (top-right) — exits the video entirely. Icon only (no circle/pill). */}
      {onClose && (
        <button
          type="button"
          onClick={(e) => {
          ...
          className="absolute right-3 top-3 z-20 flex items-center justify-center text-white drop-shadow-[0_1px_3px_rgba(0,0,0,0.6)] transition-transform hover:text-white active:scale-90"
        >
          <X className="size-7" />
        </button>
      )}

      {/* Transport cluster (rewind / play / forward), pinned to the exact center
          of the video. The wrapper spans the whole frame, so it must NOT capture
          pointer events (otherwise it would block the surface-tap toggle); only
          the actual buttons opt back in. */}
      ...
      {/* Base cluster (YouTube-style): speed pill (left) + expand (right) sit just
          above the time tracker, which is docked at the very base of the video. */}
