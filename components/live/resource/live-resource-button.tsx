...
export function LiveResourceButton() {
  ...
  return (
    <motion.button
      ...
      initial={false}
      animate={hidden ? { opacity: 0, scale: 0.8, x: 12, pointerEvents: "none" } : { opacity: 1, scale: 1, x: 0 }}
      transition={{ type: "spring", stiffness: 320, damping: 26 }}
      whileTap={{ scale: 0.92 }}
      className="pointer-events-auto absolute right-3 top-1/2 z-[10] flex size-12 -translate-y-1/2 items-center justify-center rounded-full border border-white/15 bg-zinc-900/85 text-white shadow-2xl ring-1 ring-black/40 backdrop-blur-xl"
    >
