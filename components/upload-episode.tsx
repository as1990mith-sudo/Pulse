...
import { Plus, X, Loader2, Headphones, Video, ImageIcon, UploadCloud } from "lucide-react"
...
export function UploadEpisode() {
  ...
  function submit(e: React.FormEvent) {
    ...
    startTransition(async () => {
      setUploading(true)
      try {
        const duration = await getMediaDuration(mediaFile, kind)
        const media = await uploadMedia(mediaFile, "episodes")
        const cover = coverFile ? await uploadMedia(coverFile, "covers") : null
  ...
  if (!open) {
    return (
      <Button onClick={() => setOpen(true)} className="w-full gap-2" variant="secondary">
        <Plus className="size-4" /> Upload to catalogue
      </Button>
    )
  }
  ...
  return (
    <form
      onSubmit={submit}
      className="space-y-4 rounded-2xl border border-border/60 bg-card p-4 sm:p-5"
    >
      <div className="flex items-center justify-between">
        <p className="font-semibold">Upload an episode</p>
        <button
          type="button"
          onClick={() => {
            reset()
      ...
      <div role="tablist" aria-label="Episode type" className="flex items-center gap-1 rounded-full border border-border/60 bg-background p-1">
        {(
          [
      ...
      <div>
        <button
          ...
          className="flex w-full items-center gap-3 rounded-xl border border-dashed border-border/70 bg-background px-4 py-3 text-left transition-colors hover:border-primary"
        >
      ...
      <div className="flex items-center gap-3">
        <button
          ...
          className="flex size-16 shrink-0 items-center justify-center overflow-hidden rounded-xl border border-dashed border-border/70 bg-background text-muted-foreground transition-colors hover:border-primary"
        ...
        <div className="text-sm">
          ...
          <p className="text-xs text-muted-foreground">Optional — square works best.</p>
        </div>
